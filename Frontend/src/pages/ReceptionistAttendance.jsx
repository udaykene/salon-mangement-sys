...
const ReceptionistAttendance = () => {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      // No branchId needed, backend handles it for receptionist

      const res = await api.get("/", { params });
      setStaffList(res.data);
      if (allStaff.length === 0) setAllStaff(res.data);
    } catch (err) {
  ...
  }, [reportType, reportDate, activeTab]);
            ...
            <div className="p-5">
              ...
              <div className="space-y-1.5 mb-5">
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  ...
                  <span>{user?.branchName || "Current Branch"}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
        ...
        {filteredStaffList.length === 0 && (
          <div className="col-span-full py-10 text-center text-gray-500">
            {searchQuery ? (
              ...
              <>
                No other staff members were found for your branch ({user?.branchName || "Current Branch"}).
                <br />
                <span className="text-xs italic">
                  Note: You are hidden from this list to prevent marking your own attendance.
                </span>
              </>
            )}
          </div>
